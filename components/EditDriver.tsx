...
const EditDriver = ({ onBack, driver, onSubmit }: Props): React.ReactElement => {
  const [name, setName] = useState(driver.name);
  const [phoneNumber, setPhoneNumber] = useState(driver.phoneNumber);
  const [licenseNumber, setLicenseNumber] = useState(driver.licenseNumber);
  const [licenseExpiryDate, setLicenseExpiryDate] = useState(
    driver.licenseExpiryDate ? new Date(driver.licenseExpiryDate).toLocaleDateString('en-GB') : ''
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[styles.updateButton, !isFormValid() && { opacity: 0.5 }]}
