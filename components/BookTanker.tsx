...
const BookTanker = ({ onBack, onSubmit }: Props): React.ReactElement => {
  ...
  const parseDateFromInput = (): Date | null => {
  ...
  const parseTimeFromInput = (): { hour24: number; mm: number } | null => {
  ...
  const handleSubmit = (): void => {
    ...
    const payload = { name, address, date: dateIso, time: timeIso, tankerSize, agency, comments };
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.row}>
          ...
          <View style={[styles.field, styles.rowItem]}>
            ...
            <View style={styles.timeRow}>
              ...
              <TouchableOpacity onPress={() => setMeridiem((m) => (m === 'AM' ? 'PM' : 'AM'))} activeOpacity={0.8} style={styles.ampmButton}>
        ...
        <View style={styles.field}>
          ...
          <View style={styles.quantityRow}>
            <TouchableOpacity style={[styles.quantityButton, tankerSize === '10k' && styles.quantitySelected]} onPress={() => setTankerSize('10k')} activeOpacity={0.8}>
              <Text style={styles.quantityTitle}>10,000 Liters</Text>
              <Text style={styles.quantityPrice}>₹600</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.quantityButton, tankerSize === '20k' && styles.quantitySelected]} onPress={() => setTankerSize('20k')} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  ampmButton: { marginLeft: 8, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#2A2A2A', borderRadius: 8, paddingVertical: 14, paddingHorizontal: 16 },
  ampmText: { color: '#D4AF37', fontSize: 16, fontWeight: '700' },
  row: { flexDirection: 'row', gap: 12 },
  rowItem: { flex: 1 },
  quantityRow: { flexDirection: 'row', gap: 12 },
  quantityButton: { flex: 1, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#2A2A2A', borderRadius: 12, padding: 14 },
  quantitySelected: { borderColor: '#D4AF37', backgroundColor: '#232014' },
  quantityTitle: { color: '#FFFFFF', fontSize: 16, fontWeight: '600', marginBottom: 4 },
  quantityPrice: { color: '#D4AF37', fontSize: 14, fontWeight: '600' },
  footer: { padding: 16, borderTopWidth: 1, borderTopColor: '#2A2A2A', backgroundColor: '#000000' },
  bookButton: { backgroundColor: '#D4AF37', borderRadius: 8, paddingVertical: 14, alignItems: 'center' },
  bookButtonText: { color: '#000000', fontSize: 16, fontWeight: '700' },
