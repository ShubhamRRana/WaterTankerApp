...
const BusinessReports = ({ onBack }: Props): React.ReactElement => {
  ...
  // Generate mock data based on filter type
  useEffect(() => {
    if (filterType === 'month') {
  ...
  const getTotalOrders = (): number => {
    return reportData.reduce((sum, item) => sum + item.orders, 0);
  };
    ...
    try {
      ...
      if (filterType === 'month') {
        ...
        reportData.forEach(item => {
          csvContent += `${item.day},${item.revenue},${item.orders}\n`;
        });
      } else {
      ...
      const fileName = `business_report_${filterType}_${selectedYear}_${selectedMonth + 1}.csv`;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.titleSection}>
          <Text style={styles.mainTitle}>Business Reports</Text>
          <Text style={styles.subtitle}>Analytics and insights for your water tanker business</Text>
        </View>
        ...
        <View style={styles.summarySection}>
          ...
          <View style={styles.summaryCards}>
            ...
            <View style={styles.summaryCard}>
              <Text style={styles.summaryValue}>{getTotalOrders().toLocaleString()}</Text>
          ...
          {reportData.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              <Text style={styles.tableRowLabel}>
                {filterType === 'month' ? item.day : item.month}
              </Text>
              <Text style={styles.tableRowValue}>₹{item.revenue.toLocaleString()}</Text>
              <Text style={styles.tableRowValue}>{item.orders}</Text>
            </View>
          ))}
        </View>
